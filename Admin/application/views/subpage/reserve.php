...
<meta charset="UTF-8">
    ...
    <link rel="stylesheet" href="<?php echo base_url(); ?>assets/css/reserve.css">
...
<body>
    ...
    <div class="main">
        <div class="col-1">
            ...
            <div class="search">
                <form action="<?php echo site_url('post/searchBookReserved'); ?>" method="get">
            ...
            <div class="confirmation_content">
                ...
                <div class="book-info3">
                    ...
                    <div class="confirm_btn">
                        <a href="#" class="button" id="releaseBTN">Release</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <?php
        if($this->session->flashdata('book-released')) {	?>
        <div class="book_return_class" id="book_return_class">
            <div class="container">
                <p class="text-danger text-center" id="warning-check-book" style="font-size:12px; color:Green;"><?=$this->session->flashdata('book-released')?></p>
            </div>
        </div>
    <?php } ?>
    ...
    <script>
        ...
        table.addEventListener('click', function (event) {
            ...
            const cells = row.getElementsByTagName('td'); // Get all cells in the row
                ...
                if (cells.length > 0) {
                    ...
                    document.getElementById('student_ID-p').textContent = "Student ID: " + cells[3].textContent.trim();
                    document.getElementById('student_name-p').textContent = "Student Name: " + cells[4].textContent.trim();

                    const status = cells[10].textContent.trim();

                    const originalOnClick = () => {
                        open_qr_btn(); // Call the custom function
                    };

                    if (status === 'Waiting') {
                        releaseBTN.onclick = null;
                        releaseBTN.disabled = true;
                        releaseBTN.textContent = "Waiting..."; // Optional: Update button text
                    } else {
                        releaseBTN.onclick = originalOnClick;
                        releaseBTN.disabled = false; // Enable the button if the condition is not met
                        releaseBTN.textContent = "Release"; // Optional: Reset button text
                    }

                    transaction_ID = cells[0].textContent.trim();
                    s_ID = cells[3].textContent.trim();
                }
            }
        });
        ...
        window.addEventListener('DOMContentLoaded', (event) => {
            ...
            function onScanSuccess(decodedText, decodedResult) {
                ...
                returnBook(transaction_ID, s_ID, scannedQr);
            }
        ...
        function returnBook(transaction_ID, s_ID, scannedQr) {
            if(s_ID === scannedQr){
                const url = "<?php echo base_url('Post/release_Reserved'); ?>";
                window.location.href = `${url}?transaction_ID=${encodeURIComponent(transaction_ID)}`;
            }
            else{
                alert("Qr code error!");
            }
