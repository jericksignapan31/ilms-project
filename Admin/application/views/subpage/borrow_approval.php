...
<body>
            ...
            <div class="confirmation_content">
                ...
                <div class="book-info3">
                    ...
                    <div class="confirm_btn">
                        <a href="#" onclick="open_qr_btn()" class="button">Approve</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <?php
        if($this->session->flashdata('borrow-approve')) {	?>
        <div class="book_return_class" id="book_return_class">
            <div class="container">
                <p class="text-danger text-center" id="warning-check-book" style="font-size:12px; color:Green;"><?=$this->session->flashdata('borrow-approve')?></p>
            </div>
        </div>
    <?php } ?>
    ...
    <script>
        ...
        table.addEventListener('click', function (event) {
            ...
            if (row && row.rowIndex > 0) { // Exclude header row
                ...
                if (cells.length > 0) {
                    ...
                    document.getElementById('title-p').textContent = cells[6].textContent.trim();
                    document.getElementById('author-p').textContent = "Author: " + cells[7].textContent.trim();
                    document.getElementById('edition-p').textContent = "Edition: " + cells[8].textContent.trim();
        ...
        window.addEventListener('DOMContentLoaded', (event) => {
            ...
            function onScanSuccess(decodedText, decodedResult) {
                ...
                approveBorrow(transaction_ID, s_ID, scannedQr);
            }
        ...
        function approveBorrow(transaction_ID, s_ID, scannedQr) {
            if(s_ID === scannedQr){
                const url = "<?php echo base_url('Post/approve_borrow'); ?>";
